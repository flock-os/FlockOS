...
import { ur, vine, rows, esc, snip, emptyState, backendOffline, loadingCards, sectionHead, chip } from './the_gospel_shared.js';
...
async function _load(root) {
  ...
  const V = vine();
  if (!V || !V.app || !V.app.words) { listEl.innerHTML = backendOffline('Lexicon not loaded.'); return; }
  try {
    const res = await V.app.words();
    _state.rows = rows(res);
    _paint(root);
  } catch (e) {
    console.error('[gospel/lexicon] load:', e);
    listEl.innerHTML = emptyState({ icon: '⚠️', title: 'Could not load lexicon', body: e.message || String(e) });
  }
}
